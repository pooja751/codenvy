...
public class InviteDaoTest {
  ...
  public void shouldCreateOrganizationInvite() throws Exception {
    ...
    assertEquals(
        inviteDao.getInvite(invite.getDomainId(), invite.getInstanceId(), invite.getEmail()),
        invite);
  }
  ...
  public void shouldUpdateInviteIfItIsAlreadyExistOnStoring() throws Exception {
    ...
    assertEquals(
        inviteDao.getInvite(toUpdate.getDomainId(), toUpdate.getInstanceId(), toUpdate.getEmail()),
        toUpdate);
  }

  @Test
  public void shouldReturnInvitesByEmail() throws Exception {
    List<InviteImpl> email1Invitations =
        Pages.stream((maxItems, skipCount) -> inviteDao.getInvites(EMAIL_1, maxItems, skipCount), 1)
  ...
  public void shouldReturnInvitesByWorkspace() throws Exception {
    List<InviteImpl> workspaceInvites =
        Pages.stream(
                (maxItems, skipCount) ->
                    inviteDao.getInvites(
                        WorkspaceDomain.DOMAIN_ID, workspace.getId(), skipCount, maxItems),
                1)
            .collect(Collectors.toList());
  ...
  public void shouldReturnInvitesByOrganization() throws Exception {
    List<InviteImpl> organizationInvites =
        Pages.stream(
                (maxItems, skipCount) ->
                    inviteDao.getInvites(
                        OrganizationDomain.DOMAIN_ID, organization.getId(), skipCount, maxItems),
                1)
            .collect(Collectors.toList());
  ...
  public void shouldReturnInvite() throws Exception {
    InviteImpl fetched =
        inviteDao.getInvite(OrganizationDomain.DOMAIN_ID, organization.getId(), EMAIL_1);
