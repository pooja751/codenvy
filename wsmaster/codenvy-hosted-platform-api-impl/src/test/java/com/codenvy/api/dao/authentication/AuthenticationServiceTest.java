...
import org.eclipse.che.commons.auth.AuthenticationExceptionMapper;
...
public class AuthenticationServiceTest {
  @Mock protected AuthenticationHandler handler;
  @Mock protected AuthenticationHandlerProvider handlerProvider;
  @Mock protected TicketManager ticketManager;
  @Mock protected CookieBuilder cookieBuilder;
  @Mock protected TokenGenerator uniqueTokenGenerator;
  @Mock protected UserDao userDao;

  private static final UserImpl USER =
      new UserImpl("id-123123", "user@site.com", "userSuperMonster");
  private static final UserImpl USER_2 = new UserImpl("id-444", "mini@site.com", "MiniMonster");
  @InjectMocks protected AuthenticationDaoImpl dao;
  ...
  public void init() throws NotFoundException, ServerException {
    token = "t1";
    token2 = "t2";
    when(ticketManager.getAccessTicket(eq(token2)))
    ...
    doAnswer(
            invocation -> {
              ...
              if (token1 != null && !token1.isEmpty()) {
                builder.header(
                    "Set-Cookie",
                    new NewCookie("token-access-key", token1, "/sso/server", null, null, 0, secure)
                        + ";HttpOnly");
                builder.header(
                    "Set-Cookie",
                    new NewCookie("session-access-key", token1, "/", null, null, 0, secure)
                        + ";HttpOnly");
              }
        ...
        .when(cookieBuilder)
        .setCookies(any(Response.ResponseBuilder.class), anyString(), anyBoolean());
            ...
            new Answer<Object>() {
              ...
              public Object answer(InvocationOnMock invocation) throws Throwable {
                ...
                if (token != null && !token.isEmpty()) {
                  builder.header(
                      "Set-Cookie",
                      new NewCookie("token-access-key", token, "/sso/server", null, null, 0, secure)
                          + ";HttpOnly");
                  builder.header(
                      "Set-Cookie",
                      new NewCookie("session-access-key", token, "/", null, null, 0, secure)
                          + ";HttpOnly");
                }
                builder.cookie(new NewCookie("logged_in", "true", "/", null, null, 0, secure));
                return null;
  ...
  private static <T> T unwrapDto(
      com.jayway.restassured.response.Response response, Class<T> dtoClass) {
