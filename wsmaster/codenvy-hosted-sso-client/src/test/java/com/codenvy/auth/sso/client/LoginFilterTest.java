...
public class LoginFilterTest {
  ...
  @Mock RequestTokenExtractor tokenExtractor;
  @Mock ClientUrlExtractor clientUrlExtractor;
  @Spy RequestWrapper requestWrapper = new RequestWrapper();
  @Mock ServerClient ssoServerClient;
  ...
  @Mock RequestFilter requestFilter;
  @Mock TokenHandler tokenHandler;

  @InjectMocks LoginFilter filter;
  ...
  public void shouldRedirectToSsoServerIfSessionDoesntContainsPrincipalOnGet()
    ...
    when(clientUrlExtractor.getClientUrl(eq(request)))
        .thenReturn("http://localhost:8080/ws/myworkspace");
    setFieldValue(
        filter,
        "tokenHandler",
    ...
    verify(response)
        .sendRedirect(
            eq(
                "/api/internal/sso/server/refresh?"
    ...
    request
        ...
        .setAttribute(
            ...
            new SsoClientPrincipal(
                "token",
                "http://localhost:8080/ws/mypersonal" + ".Workspace",
  ...
  public void shouldBeAbleToReplaceAnonymousWithCorrectUserPrincipal()
    ...
    when(ssoServerClient.getSubject(eq("t13f"), anyString()))
        .thenReturn(createSubject("user@domain"));
    when(ssoServerClient.getSubject(eq("t12f"), anyString()))
        .thenReturn(createSubject("Anonymous123@domain"));
    ...
    assertEquals(
        ((HttpServletRequest) captor.getValue()).getUserPrincipal().getName(), "user@domain");
  }

  @Test
  public void shouldRedirectToRefreshTokenIfTokenIsInvalidAndRecoverConfigured()
      throws IOException, ServletException {
    HttpServletRequest request =
        new MockHttpServletRequest("http://localhost:8080/ws/ws?token=t13f", null, 0, "GET", null);
    when(clientUrlExtractor.getClientUrl(eq(request))).thenReturn("http://localhost:8080/ws/ws");
  ...
  public void shouldReuseSessionAssociatedWithToken() throws IOException, ServletException {
    ...
    HttpServletRequest request =
        spy(
            new MockHttpServletRequest(
                "http://localhost:8080/ws/ws?token=t13f", null, 0, "GET", null));
  ...
  public void shouldRespond401IfPostRequestHasNoToken() throws IOException, ServletException {
    ...
    assertEquals(
        ...
        "{\"message\":\"User not authorized to call this method.\"}");
  }
  ...
  @Test
  public void shouldRespond403ForPostRequestWithInvalidToken()
