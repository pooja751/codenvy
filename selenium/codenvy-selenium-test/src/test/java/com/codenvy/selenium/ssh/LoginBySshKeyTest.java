...
public class LoginBySshKeyTest {
  ...
  private static final String CHECKED_FILE_NAME =
      "checkedFile" + new Random().nextInt(9999) + ".txt";
  ...
  public void setUp() throws Exception {
    URL resource =
        LoginBySshKeyTest.this.getClass().getResource("/projects/default-spring-project");
    testProjectServiceClient.importProject(
        ws1.getId(), Paths.get(resource.toURI()), PROJECT_NAME, ProjectTemplates.MAVEN_SPRING);
    testProjectServiceClient.createFileInProject(
        ws1.getId(), PROJECT_NAME, CHECKED_FILE_NAME, "text");

    ide.open(ws1);
    String commandForPreparingSshDirectory = "mkdir -p /home/user/.ssh";
    testCommandServiceClient.createCommand(
        commandForPreparingSshDirectory,
        NAME_COMMAND_FOR_CREATION_SSH_DIRECTORY,
  ...
  public void loginBySshKeyTest() throws Exception {
    ...
    testCommandServiceClient.createCommand(
        commandForCreatingSshKey,
        NAME_COMMAND_FOR_CREATION_SSH_KEY,
        TestCommandsConstants.CUSTOM,
        ws2.getId());
    consoles.clickOnOpenSshTerminalBtn();
    testCommandServiceClient.createCommand(
        "chmod 0700 "
            ...
            + PROJECT_NAME
            + " && ls \" ",
        NAME_COMMAND_CONNECTION_BY_SSH_KEY,
        TestCommandsConstants.CUSTOM,
        ws2.getId());
    ...
    new WebDriverWait(seleniumWebDriver, 10)
        .until(ExpectedConditions.invisibilityOfElementLocated(By.id("gwt-debug-glass-panel")));
  ...
  /**
   * Match and get command for ssh connection from the SSH console and add command for strict host
   * key
   *
  ...
  private String getCommandFromSshConsole() {
    ...
    if (matcher.find()) {
      return (matcher.group().replace("[", ""))
          + " "
          + PATH_TO_SSH_KEY_FOLDER
          + strictHostKeyCheckCommand;
    } else
      throw new RuntimeException(
          "Cannot find the instruction about SSH connection in SSH command console. "
   ...
   *
   * @param command the name of launched command
   */
